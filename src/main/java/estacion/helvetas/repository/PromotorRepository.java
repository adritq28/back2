...
public interface PromotorRepository extends JpaRepository<Usuario, Integer> {
    ...
    @Query(value = "SELECT DISTINCT ON (u.id_usuario) u.id_usuario AS id, m.nombre AS nombreMunicipio, z.nombre AS nombreZona, "
            +
            "CONCAT(u.nombre, ' ', u.ape_pat, ' ', COALESCE(u.ape_mat, '')) AS nombreComp, u.telefono AS telefono, z.id_zona AS idZona "
            +
            "FROM usuario u " +
    ...
    @Query("SELECT u.idUsuario, m.nombre, z.nombre, " +
            ...
            "join Municipio m on  m.idMunicipio=z.idMunicipio WHERE u.idUsuario = :idUsuario")
